...
export default {
  ...
  methods: {
    ...
    F(rightHalf, k) {
      ...
      const xored = (parseInt(expanded, 2) ^ parseInt(k, 2))
        .toString(2)
        .padStart(8, "0");
    ...
    encryptChar(char, k1, k2) {
      char = this.permute(char, this.IP);
      let l0 = char.substring(0, 4);
      let r0 = char.substring(4);
      const l1 = r0;
      let fResult = this.F(r0, k1);
      let r1 = (parseInt(l0, 2) ^ parseInt(fResult, 2))
        .toString(2)
        .padStart(4, "0");
      fResult = this.F(r1, k2);
      let r2 = (parseInt(l1, 2) ^ parseInt(fResult, 2))
        .toString(2)
        .padStart(4, "0");
      return this.permute(r2 + r1, this.IPni);
    },
    decryptChar(char, k1, k2) {
      char = this.permute(char, this.IP);
      let r2 = char.substring(0, 4);
      let l2 = char.substring(4);
      let fResult = this.F(l2, k2);
      let l1 = (parseInt(r2, 2) ^ parseInt(fResult, 2))
        .toString(2)
        .padStart(4, "0");
      fResult = this.F(l1, k1);
      let r1 = (parseInt(l2, 2) ^ parseInt(fResult, 2))
        .toString(2)
        .padStart(4, "0");
    ...
    encrypt(plaintext, k1, k2) {
      ...
      for (let i = 0; i < plaintext.length; i++) {
        ...
        const encryptedChar = this.encryptChar(binaryChar, k1, k2);
        const encryptedCharCode = parseInt(encryptedChar, 2);
        const encryptedCharValue = String.fromCharCode(encryptedCharCode);
        ciphertext += encryptedCharValue;
      }
    ...
    decrypt(ciphertext, k1, k2) {
      ...
      for (let i = 0; i < ciphertext.length; i++) {
        ...
        const charCode = char.charCodeAt(0);
        const binaryChar = charCode.toString(2).padStart(8, "0");
        const decryptedChar = this.decryptChar(binaryChar, k1, k2);
        const decryptedCharCode = parseInt(decryptedChar, 2);
        const decryptedCharValue = String.fromCharCode(decryptedCharCode);
        plaintext += decryptedCharValue;
      }
      this.decryptedText = plaintext;
    },
    encryptText() {
      const { k1, k2 } = this.generateKey(this.key, this.P10, this.P8);
      this.encrypt(this.plaintext, k1, k2);
    },

    decryptText() {
  console.log("解密按钮被点击");
  const { k1, k2 } = this.generateKey(this.key, this.P10, this.P8);
  this.decrypt(this.ciphertext, k1, k2);
},
